...
def train_RGCN(params):
    ...
    if params["write_MSE"] or params["write_NSE"] or params["save_wbe"]:
        path = "results_gnn/" + datetime.now().strftime("%Y_%m_%d")
        ...
        if params["write_MSE"]:
            file_name_mse = "results_gnn/" + datetime.now().strftime("%Y_%m_%d") +'/'+datetime.now().strftime("D%Y_%m_%d_T%H_%M") +'_' +params['vars']+'_' +params['region_test']+ '_mse_b.txt'
        if params["write_NSE"]:
            file_name_nse = "results_gnn/" + datetime.now().strftime("%Y_%m_%d") +'/'+datetime.now().strftime("D%Y_%m_%d_T%H_%M") +'_' +params['vars']+'_' +params['region_test']+ '_nse_b.txt'
        if params["save_wbe"]:
            file_name_wbe = "results_gnn/" + datetime.now().strftime("%Y_%m_%d") +'/'+datetime.now().strftime("D%Y_%m_%d_T%H_%M") +'_' +params['vars']+'_' +params['region_test']+ '_wbe_b.csv'
    vars =  params['vars']
    ...
    for epoch in range(epochs):
        ...
        for batch in dataset_train_list_batch:
            ...
            general_train_loss += mse_model_train_batch 
        #print(loss_nsl)
...
# gnn hps
regions_dicts_all = [{'region':'Boise_river', 'years_train':26, 'years_test':9, 'epochs':100, 'hidden':256, 'lr':0.001, 'wd':0.005}]
