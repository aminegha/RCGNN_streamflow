...
def get_IW_weights(mse_list, K):
    max_loss = torch.max(mse_list)
    bin_interval = max_loss / K
    bin_ranks = torch.ceil(mse_list / bin_interval).clamp(max=K).long()
    bin_counts = torch.histc(bin_ranks.float(), bins=K, min=1, max=K)
    bin_percentages = bin_counts / len(mse_list)
    element_percentages = bin_percentages[bin_ranks - 1]
    weights = bin_ranks.float()/element_percentages
    return weights
...
class RecurrentGCN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight, h):
        ...
        #h = self.LeakyReLU(h)
        return h, h_0, latent_feats
        #return h, h_0, c_0

def water_balance(v): #y = [1,3,2]
    b = v[:,0] == 0
    return b
...
def train_RGCN(params):
    ...
    if params["write_MSE"] or params["write_NSE"] or params["save_wbe"]:
        path = "results_gnn_nsl_WI/" + datetime.now().strftime("%Y_%m_%d")
        ...
        if params["write_MSE"]:
            file_name_mse = "results_gnn_nsl_IW/" + datetime.now().strftime("%Y_%m_%d") +'/'+datetime.now().strftime("D%Y_%m_%d_T%H_%M") +'_' +params['vars']+'_' +params['region_test']+ '_mse_WI.txt'
        if params["write_NSE"]:
            file_name_nse = "results_gnn_nsl_IW/" + datetime.now().strftime("%Y_%m_%d") +'/'+datetime.now().strftime("D%Y_%m_%d_T%H_%M") +'_' +params['vars']+'_' +params['region_test']+ '_nse_WI.txt'
        if params["save_wbe"]:
            file_name_wbe = "results_gnn_nsl_IW/" + datetime.now().strftime("%Y_%m_%d") +'/'+datetime.now().strftime("D%Y_%m_%d_T%H_%M") +'_' +params['vars']+'_' +params['region_test']+ '_wbe_WI.csv'
    vars =  params['vars']
    ...
    for epoch in range(epochs):
        ...
        for batch in dataset_train_list_batch:
            ...
            batch_precipitation =  torch.cat([torch.unsqueeze(torch.mean(batch_feats[i], 0)[0],0) for i in range(len(batch))])
            ...
            batch_et = torch.cat([torch.unsqueeze(batch[i][1].et,0) for i in range(len(batch))])
            batch_output, h, latent_train_tensor = model(batch_feats, batch[0][1].edge_index, batch[0][1].edge_attr, h)
            latent_train_list.append(latent_train_tensor)
            mse_model_train_batch = F.mse_loss(batch_output*train_std+train_mean, batch_y*train_std+train_mean)
            mse_vic_train_batch = F.mse_loss(batch_vic, batch_y*train_std+train_mean)
            ...
            ce_term_pred = torch.sum(batch_precipitation) - torch.sum(batch_et) # precip (mean(spatial), sum(temporal))- et
            y_const_pred = torch.unsqueeze(torch.stack((torch.sum(batch_output*train_std+train_mean), ce_term_pred)),1) # (output, ce_term)
            loss_wbe = water_balance_constraint(y_const_pred.unsqueeze(0))
            # IW: 
            #mse_list = [F.mse_loss(output * train_std + train_mean, y * train_std + train_mean).item() for output, y in zip(batch_output, batch_y)]
            mse_list = F.mse_loss(batch_output*train_std+train_mean, batch_y*train_std+train_mean, reduction='none')
            IW_weights = get_IW_weights(mse_list, 10)
            IW_loss_set = mse_list * IW_weights
            IW_loss = torch.mean(IW_loss_set)
            general_train_loss += mse_model_train_batch + params['lambda_wbe'] * loss_wbe + params['lambda_IW'] * IW_loss
        ...
        for batch in dataset_test_list_batch:
            ...
            batch_y = torch.cat([torch.unsqueeze(batch[i][1].y,0) for i in range(len(batch))])
            batch_vic = torch.cat([torch.unsqueeze(batch[i][1].vics,0) for i in range(len(batch))])
            batch_output, h, latent_test_tensor = model(batch_feats, batch[0][1].edge_index, batch[0][1].edge_attr, h)
            latent_test_list.append(latent_test_tensor)
    ...
    latent_results = {}
    latent_results['latent_train'] = torch.cat(latent_train_list, 0)
    latent_results['latent_test'] = torch.cat(latent_test_list, 0)
    latent_results['y_train'] = torch.tensor(train_targets_real).float()
    latent_results['y_test'] = torch.tensor(test_targets_real).float()
    if params["save_latent"]:
        with open('results_latent/latent_'+ params['region_test'] +'.pickle', 'wb') as h:
            pickle.dump(latent_results, h)
    return model_results

torch.manual_seed(0)
random.seed(0)
np.random.seed(0)
water_balance_constraint = constraint(water_balance, SatisfactionBruteForceSolver())

results_directory = "results_gnn_nsl_wi"
# Check whether the specified path exists or not
...
regions_dicts_all = [{'region':'Yakima', 'years_train':26, 'years_test':9, 'epochs':100, 'hidden':256, 'lr':0.01, 'wd':0.005}]
